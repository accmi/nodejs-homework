...
import { userValidationSchema } from './validator';
...
class UserRouter {
    ...
    createUser(req: RequestType<UserType>, res: Response, next: NextFunction) {
        const result = createUser(req.body);

        next(result);
    }

    updateUser(req: RequestType<UserType>, res: Response, next: NextFunction) {
        const result = updateUser(req.body);

        next(result);
    }

    deleteUser(req: RequestType<{}, GetUserByIdType>, res: Response, next: NextFunction) {
        const result = deleteUser(req.query.id);

        next(result);
    }

    getUser(req: RequestType<{}, GetUserByIdType>, res: Response, next: NextFunction) {
        const result = getUserById(req.query.id);

        next(result);
    }

    getUsers(req: RequestType<{}, GetUsersType>, res: Response, next: NextFunction) {
        const { query: { loginSubstring, limit } } = req;
        const result = getUsers(loginSubstring, limit);
